...
class CustomNotificationSaveCacheManager {
  static CustomNotificationSaveCacheManager? _instance;
  static CustomNotificationSaveCacheManager get instance =>
      _instance ??= CustomNotificationSaveCacheManager._();

  CustomNotificationSaveCacheManager._() {
    key = "bildirim_list_cache_manager_new";
  }
  ...
  Future<void> removeItem(String key, int index) async {
    List<dynamic>? list = box?.get(key);
    await list?.removeAt(index);

    await box?.put(key, list ?? []);
  }

  Future<void> removeAllList(String key) async {
